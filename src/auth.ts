...
export async function auth(): Promise<string> {
  try {
    ...
    const login_url = redirectLinkParser(
      await input(
        "Please jump to the following URL, copy the URL starting with 'np5c38e31cd085304b://' and paste it into the standard input: ",
      ),
    );
    ...
    const session_token = (
      await getSessionToken(session_token_code, code_verifier)
    ).unwrap();
