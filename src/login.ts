...
): Promise<Option<{ id_token: string; access_token: string }>> {
  const params = {
    client_id: "5c38e31cd085304b",
...
): Promise<Option<{ language: string; birthday: string; country: string }>> {
  const params = {
    Accept: "application/json",
    "Accept-Encoding": "gzip",
    "Accept-Language": "en-US",
...
async function fAPI(
  service_id_token: string,
): Promise<Option<{ request_id: string; timestamp: number; f: string }>> {
  const params = {
